...
public class MessageBookshelfSync extends SerializableMessage {
    ...
    public IMessage handleMessage (MessageContext context) {
        ...
        Minecraft.getMinecraft().addScheduledTask( () -> {
            ...
            final TileEntityAdvancedTable tile = (TileEntityAdvancedTable) world.getTileEntity(MessageBookshelfSync.this.tablePos);
            tile.bookshelves.clear();
            for (int i = 0; i < MessageBookshelfSync.this.pos.length; i++) {
                tile.bookshelves.put(MessageBookshelfSync.this.pos[i], MessageBookshelfSync.this.enchantability[i]);
            }
        });
