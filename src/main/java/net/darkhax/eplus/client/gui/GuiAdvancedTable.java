...
//
// private static final int guiOffset = 26;
//
// private static final ResourceLocation TEXTURE = new ResourceLocation("eplus",
// "textures/gui/enchant.png");
// private static final ResourceLocation BUTTON_ENCHANT = new ResourceLocation("eplus",
// "textures/gui/button_enchant.png");
// private static final ResourceLocation BUTTON_REPAIR = new ResourceLocation("eplus",
// "textures/gui/button_repair.png");
//
...
// }
//
// this.sliderIndex = this.sliding ? Math.round(tempY / 57D * this.enchantingPages / .25) * .25
// : this.sliderIndex;
//
...
// }
//
// final double sliderY = this.sliding ? tempY : 57 * (this.sliderIndex /
// this.enchantingPages);
//
// GlStateManager.color(1.0f, 1.0f, 1.0f);
// this.drawTexturedModalRect(this.guiLeft + guiOffset + 180, this.guiTop + 16 + (int) sliderY,
// 0, 182, 12, 15);
//
...
//
// information.add(this.fontRenderer.listFormattedStringToWidth(String.format("%s: %s",
// I18n.format("tooltip.eplus.playerlevel"), this.player.experienceLevel), maxWidth));
//
// if (!stack.isEmpty()) {
// if (this.hasLevelChanged()) {
//
// final boolean exp = this.totalCost <= EnchantmentUtils.getExperienceFromLevel(1) &&
// this.totalCost >= -EnchantmentUtils.getExperienceFromLevel(1);
// final boolean negExp = this.totalCost < 0;
// final int finalCost = exp ? this.totalCost : negExp ?
// -EnchantmentUtils.getLevelsFromExperience(-this.totalCost) :
// EnchantmentUtils.getLevelsFromExperience(this.totalCost);
// information.add(this.fontRenderer.listFormattedStringToWidth(String.format("%s: %s", exp ?
// I18n.format("tooltip.eplus.experienceGained") : I18n.format("tooltip.eplus.enchant"),
// finalCost), maxWidth));
// }
//
// else if (ConfigurationHandler.allowRepairs && stack.isItemEnchanted() &&
// stack.isItemDamaged()) {
// information.add(this.fontRenderer.listFormattedStringToWidth(I18n.format("tooltip.eplus.repair",
// EnchantmentUtils.getExpForDisplay(this.totalCost)), maxWidth));
// }
// }
//
// information.add(this.fontRenderer.listFormattedStringToWidth(String.format("%s: %s",
// I18n.format("tooltip.eplus.maxlevel"),
// EnchantmentUtils.getEnchantingPower(this.container.world, this.container.pos)), maxWidth));
//
// if (this.container.getItem().isEmpty()) {
// information.add(this.fontRenderer.listFormattedStringToWidth(I18n.format("tooltip.eplus.additem"),
// maxWidth));
// }
// else if (this.enchantments != null && this.enchantments.size() == 0 &&
// ConfigurationHandler.useQuestMode) {
// information.add(this.fontRenderer.listFormattedStringToWidth(I18n.format("tooltip.eplus.noench"),
// maxWidth));
// }
//
// for (final List<String> display : information) {
//
// int height = information.indexOf(display) == 0 ? this.guiTop + this.fontRenderer.FONT_HEIGHT
// + 8 : this.guiTop + (this.fontRenderer.FONT_HEIGHT + 8) * (information.indexOf(display) +
// 1);
//
...
// ModUtils.getModName(label.enchantment));
...
// - this.guiTop;
//
// if (eventDWheel != 0 && (mouseX >= 35 + guiOffset && mouseX <= this.xSize + guiOffset - 32
// || mouseX >= 180 + guiOffset && mouseX <= 192 + guiOffset)) {
...
//
// super.initGui();
// this.buttonList.add(new GuiGraphicButton(0, this.guiLeft + guiOffset + 9, this.guiTop + 38,
// BUTTON_ENCHANT));
// this.buttonList.add(new GuiGraphicButton(1, this.guiLeft + guiOffset + 9, this.guiTop + 63,
// BUTTON_REPAIR));
// this.dirty = true;
// }
//
...
// this.updateEnchantmentState(this.container.getEnchantments());
//
// this.handleChangedScreenSize(enchantments);
// this.updateEnchantmentLabels();
// this.enchantingPages = this.enchantmentList.size() / 4.0 > 1 ? this.enchantmentList.size() /
// 4.0 - 1.0 : 0;
// this.totalCost = 0;
...
//
// if (!label.locked && label.currentLevel > level) {
//
// int cost = this.totalCost + this.container.enchantmentCost(label.enchantment,
...
// label.dragging = false;
// label.currentLevel--;
// cost = this.totalCost + this.container.enchantmentCost(label.enchantment,
// label.currentLevel, level);
//
...
//
// if (this.dirty) {
// final List<GuiEnchantmentLabel> labels = this.getEnchantmentLabels(enchantments, 35 +
// guiOffset + this.guiLeft, 15 + this.guiTop);
//
// for (final GuiEnchantmentLabel label : this.enchantmentList) {
...
// !EnchantmentUtils.areEnchantmentsCompatible(mainLabel.enchantment, otherLabel.enchantment))
// {
...
// ConfigurationHandler.maxEnchantmentAmount > 0 && labelIndex >=
// ConfigurationHandler.maxEnchantmentAmount) {
...
// */
// private List<GuiEnchantmentLabel> getEnchantmentLabels (final Map<Enchantment, Integer> map,
// int x, int y) {
//
...
//
// labels.add(new GuiEnchantmentLabel(this.container, enchantment, map.get(enchantment), x,
// yPos));
// pixelHeight++;
...
// */
// private Map<Enchantment, Integer> updateEnchantmentState (final Map<Enchantment, Integer>
// enchantments) {
//
// if (this.enchantments != enchantments) {
//
// this.enchantments = enchantments;
// this.enchantmentList = this.getEnchantmentLabels(enchantments, 35 + guiOffset +
// this.guiLeft, 15 + this.guiTop);
