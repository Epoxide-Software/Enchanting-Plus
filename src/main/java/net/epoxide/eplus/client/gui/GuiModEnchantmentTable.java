...
public class GuiModEnchantmentTable extends GuiContainer {
    ...
    private void handleChangedEnchantment (Map<Integer, Integer> enchantments, GuiEnchantmentLabel label) {
        ...
        if (!label.locked && label.enchantmentLevel > level) {
            ...
            if ((EnchantHelper.hasRestriction(container.tableInventory.getStackInSlot(0)) && EnchantHelper.restrictionMatches(container.tableInventory.getStackInSlot(0),
                    player))/* || EPlusConfigurationHandler.allowDisenUnowned */) {
