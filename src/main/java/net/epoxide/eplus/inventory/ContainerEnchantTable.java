...
public class ContainerEnchantTable extends Container {
    ...
    public boolean canPurchase (EntityPlayer player, int cost) {
        ...
        if (EPlusConfigurationHandler.needsBookShelves) {
            if (cost > bookCases()) {
                player.addChatMessage(new ChatComponentText(StatCollector.translateToLocal("chat.eplus.morebooks") + " " + cost));
                return false;
            }
        }

        if (player.experienceLevel < cost) {
            player.addChatMessage(new ChatComponentText(StatCollector.translateToLocal("chat.eplus.morelevels") + " " + cost));
            return false;
        }
        return true;
    }
    ...
    public void enchant (EntityPlayer player, HashMap<Integer, Integer> map, int cost) throws Exception {
        ...
        if (canPurchase(player, serverCost)) {
            ...
            if (!player.capabilities.isCreativeMode)
                player.addExperience(-cost*player.xpBarCap());
        }
